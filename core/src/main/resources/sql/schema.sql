...
-- 14. 用户 AI 工具放行策略（持久化，每用户一行）
CREATE TABLE IF NOT EXISTS user_ai_policy (
    user_id VARCHAR(50) PRIMARY KEY,
    allowed_tool_types TEXT NOT NULL DEFAULT '',
    update_time DATETIME NOT NULL
);
...
CREATE TABLE IF NOT EXISTS ai_threads (
    ...
    -- 父子关系、执行模式；profile 为历史存储列，业务逻辑不再读取。
...
-- 19. AI 子 Agent 调用记录（父会话→子会话的派发关系）
