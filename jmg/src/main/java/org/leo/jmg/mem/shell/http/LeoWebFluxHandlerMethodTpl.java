...
public class LeoWebFluxHandlerMethodTpl {
    ...
    public Mono<Void> invoke(final ServerWebExchange exchange) {
        ...
        return DataBufferUtils.join(exchange.getRequest().getBody()).flatMap(buffer -> {
            try {
                ...
                buffer.read(request);
                DataBuffer output = exchange.getResponse().bufferFactory().wrap(invokeCore(request));
